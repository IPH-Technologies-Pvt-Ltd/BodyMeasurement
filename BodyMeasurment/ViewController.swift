...
//  BodyMeasurment
//
//  Created by iPHTech21 on 02/01/24.
//
...
class ViewController: UIViewController, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var bodyMeasure:UIToolbar!
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
      bodyMeasure.layer.masksToBounds = true
      bodyMeasure.layer.cornerRadius = 12
...
//        videoCameraButton.isEnabled = true
      ...
      let defaultRow = 0//(DetectorPickerRow.rowsCount / 2) - 1
...
//            let mouthWidth = self.calculateDistance(first: pose.landmark(ofType: .mouthLeft).position, second: pose.landmark(ofType: .mouthRight).position)
//
//            let faceWidth = self.calculateDistance(first: pose.landmark(ofType: .leftEar).position, second: pose.landmark(ofType: .rightEar).position)
//
//            let leftEyeWidth = self.calculateDistance(first: pose.landmark(ofType: .leftEyeOuter).position, second: pose.landmark(ofType: .leftEyeInner).position)
            
            let shoulderWidth = self.calculateDistance(first: pose.landmark(ofType: .leftShoulder).position, second: pose.landmark(ofType: .rightShoulder).position)
            
            let armLength = (self.calculateDistance(first: pose.landmark(ofType: .leftShoulder).position, second: pose.landmark(ofType: .leftElbow).position) + self.calculateDistance(first: pose.landmark(ofType: .rightShoulder).position, second: pose.landmark(ofType: .rightElbow).position))/2
            
            let handLength = (self.calculateDistance(first: pose.landmark(ofType: .leftElbow).position, second: pose.landmark(ofType: .leftWrist).position) + self.calculateDistance(first: pose.landmark(ofType: .rightElbow).position, second: pose.landmark(ofType: .rightWrist).position))/2
            
            
          self.annotationOverlayView.addSubview(poseOverlayView)
          self.resultsText = """
\nShoulder:\t\(shoulderWidth.rounded()/5) cm
\nUpper arm:\t\t\(armLength.rounded()/5) cm
\nLower arm:\t\t\(handLength.rounded()/10) cm
"""
            
//          self.showResults()
            self.label1.text = "Shoulder:\(shoulderWidth.rounded()/5) cm"
            self.label2.text = "Upper arm:\(armLength.rounded()/5) cm"
            self.label3.text = "Lower arm:\(handLength.rounded()/5) cm"
    ...
    func calculateDistance(first: Vision3DPoint, second: Vision3DPoint) -> Double {
  ...
  private func detectTextOnDevice(image: UIImage?, detectorType: DetectorPickerRow) {
    ...
    let onDeviceTextRecognizer = TextRecognizer.textRecognizer(options: TextRecognizerOptions.init())
    // [END init_text]
...
private enum Constants {
  static let images = [
    "grace_hopper.jpg", "chinese_sparse.png", "chinese.png",
